...
function ViewApplications() {
    ...
    const fetchCompanyJobApplications = async () => {
        try {
            //console.log("Calling /api/company/applicants with token:", companyToken);
                                    ...
                                    {applicant.status === "Pending" ?
                                        <div className='relative inline-block text-left group'>
                                            ...
                                            <div className='z-10 hidden absolute right-0 md:left-0 top-0 mt-2 w-32 bg-white border border-gray-200 rounded shadow group-hover:block'>
                                                <button onClick={() => changeJobApplication(applicant._id, 'Accepted')} className='block w-full text-left px-4 py-2 text-green-500 hover:bg-gray-100'>Accept</button>
                                                <button onClick={() => changeJobApplication(applicant._id, 'Rejected')} className='block w-full text-left px-4 py-2 text-red-500 hover:bg-gray-100'>Reject</button>
                                            </div>
                                        </div>
